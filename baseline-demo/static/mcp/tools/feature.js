...
import { tokenize, getTermFrequencies, cosineSimilarity, } from "../lib/text-processing.js";
import { z } from "zod";
import { resultOrEmptyText } from "../lib/mcp.js";
import { findFeaturesInContent } from "../lib/feature.js";
export const registerFeatureTools = (server) => {
    server.registerTool("list-features", {
        description: "Returns a list of all available baseline web features. Baseline features are a set of web platform features that are at different levels of support across major browsers.",
    }, async ({}) => {
        return {
            content: [
                {
    ...
    server.registerTool("discouraged-features", {
    ...
    server.registerTool("get-feature-details", {
    ...
    server.registerTool("get-feature-by-status", {
        ...
        inputSchema: {
            status: z
                ...
                .describe('The baseline status to filter by: "high", "low", or false.'),
        },
    }, async ({ status }) => {
        const results = [];
        for (const featureId in features) {
    ...
    server.registerTool("get_features_by_caniuse_id", {
        ...
        for (const featureId in features) {
            const feature = features[featureId];
            if ("caniuse" in feature &&
                feature.caniuse &&
                feature.caniuse.includes(caniuseId)) {
                results.push({ id: featureId, name: feature.name });
            }
        }
        return resultOrEmptyText(results, `No features found for caniuse ID "${caniuseId}". Please ensure the ID is correct.`);
    });
    server.registerTool("get_features_with_spec_url", {
        ...
        return resultOrEmptyText(results, `No features found for spec URL "${specUrl}". Please ensure the URL is correct.`);
    });
    server.registerTool("get_features_by_compat_feature", {
        ...
        return resultOrEmptyText(results, `No features found for compat feature "${compatFeature}". Please ensure the feature key is correct.`);
    });
    server.registerTool("get_baseline_high_since", {
        ...
        return resultOrEmptyText(results, `No features found that reached "baseline high" status since ${dateString}. Please ensure the date is correct and in YYYY-MM-DD format.`);
    });
    server.registerTool("get_baseline_low_since", {
        ...
        return resultOrEmptyText(results, `No features found that reached "baseline low" status since ${dateString}. Please ensure the date is correct and in YYYY-MM-DD format.`);
    });
    server.registerTool("get_baseline_low_last_30_days", {
        ...
        return resultOrEmptyText(results, "No features found that reached 'baseline low' status in the last 30 days.");
    });
    server.registerTool("get_baseline_high_last_30_days", {
        ...
        return resultOrEmptyText(results, "No features found that reached 'baseline high' status in the last 30 days.");
    });
    server.registerTool("get_baseline_status_changes_last_30_days", {
        ...
        const sortedResults = results.sort((a, b) => new Date(b.change_date).getTime() -
            new Date(a.change_date).getTime());
        return resultOrEmptyText(sortedResults, "No features found that changed their baseline status in the last 30 days.");
    });
    server.registerTool("get_features_by_description_keyword", {
        ...
        return resultOrEmptyText(results, `No features found with the keyword "${keyword}".`);
    });
    server.registerTool("get_features_by_name_keyword", {
    ...
    server.registerTool("get_features_with_multiple_spec_urls", {
        ...
        return resultOrEmptyText(results, "No features found with more than one specification URL.");
    });
    server.registerTool("get_features_with_no_spec_url", {
        description: "Returns web features that have no specification URL. This may indicate that a feature is non-standard.",
    }, async ({}) => {
        const results = [];
        for (const featureId in features) {
            const feature = features[featureId];
        ...
        return resultOrEmptyText(results, "No features found with no specification URL.");
    });
    server.registerTool("get_features_with_alternatives", {
        ...
        for (const featureId in features) {
            const feature = features[featureId];
            if ("discouraged" in feature && feature.discouraged) {
                if ("alternatives" in feature.discouraged &&
        ...
        return resultOrEmptyText(results, "No features found with recommended alternatives.");
    });
    server.registerTool("get_feature_alternative", {
    ...
    server.registerTool("get_random_feature", { description: "Returns a random web feature. Useful for discovery." }, async ({}) => {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify(features[randomId]),
                },
            ],
        };
    });
    server.registerTool("list_features_in_file", {
    ...
    server.registerTool("suggest_baseline_feature", {
        ...
        for (const featureId in features) {
            const feature = features[featureId];
            if (!("name" in feature) || !("description" in feature))
                continue;
            const name = feature.name || "";
            const description = feature.description || "";
            // Keyword matching
            ...
            const score = cosineSimilarity(queryTf, getTermFrequencies(tokenize(name + " " + description)));
            if (score > 0) {
        ...
        return resultOrEmptyText(combinedResults, `No features found matching the query "${query}".`);
    });
};
