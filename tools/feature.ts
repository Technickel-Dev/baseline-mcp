...
// const processedFeatures: { [key: string]: ReturnType<typeof getTermFrequencies> } = {};
    ...
    async ({}) => {
      return {
        content: [
          {
            type: "text",
            text: Object.keys(features)
              .map((featureId) => {
                const feature = features[featureId];
                // Use 'name' if it exists, otherwise fallback to an empty string
                return `${featureId}: ${"name" in feature ? feature.name : ""}`;
              })
              .join("\n"),
          },
        ],
...
// const getFeatureDetails = new Tool("get_feature_details", {
//   description:
...
// const getBaselineHighLast30DaysTool = new Tool(
...
// const getBaselineStatusChangesLast30DaysTool = new Tool(
...
// const getMinBrowserSupportForFileTool = new Tool(
