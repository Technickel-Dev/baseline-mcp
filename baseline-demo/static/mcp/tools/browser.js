...
export const registerBrowserTools = (server) => {
    server.registerTool("get_latest_browser_versions", {
    ...
    server.registerTool("check_feature_support", {
        ...
        inputSchema: {
            ...
            browserName: z
                .string()
                .describe("The name of the browser to check (e.g., 'chrome', 'firefox')."),
        },
    ...
    server.registerTool("get_unsupported_features", {
        ...
        for (const featureId in features) {
            ...
            if ("status" in feature &&
                feature.status &&
                feature.status.support &&
                !feature.status.support[browserName]) {
                results.push({ id: featureId, name: feature.name });
            }
        }
        return resultOrEmptyText(results, `All features are supported by ${browserName}.`);
    });
    server.registerTool("get_recently_supported_features", {
        ...
        inputSchema: {
            ...
            version: z
                .string()
                .describe("The version of the browser to check for new feature support (e.g., '89.0')."),
        },
        ...
        for (const featureId in features) {
            ...
            if ("status" in feature &&
                feature.status &&
                feature.status.support &&
                feature.status.support[browserName] === version) {
                results.push({ id: featureId, name: feature.name });
            }
        }
        return resultOrEmptyText(results, `No features were newly supported in ${browserName} version ${version}.`);
    });
    server.registerTool("compare_browser_support", {
        ...
        inputSchema: {
            featureIds: z
                .array(z.string())
                .describe("An array of web feature IDs to compare (e.g., ['webgl', 'canvas'])."),
        },
    }, async ({ featureIds }) => {
        const results = {};
        ...
        return resultOrEmptyText(Object.entries(results).map(([id, support]) => ({ id, support })), "No valid features found for comparison.");
    });
    server.registerTool("get_browser_release_dates", {
        ...
        return resultOrEmptyText(browser.releases, `No release data found for browser '${browserName}'.`);
    });
    server.registerTool("get_browser_release_date", {
    ...
    server.registerTool("get_features_not_supported_by_any_browser", {
        ...
        return resultOrEmptyText(results, "All features are supported by at least one major browser.");
    });
    server.registerTool("generate_browser_support_matrix", {
        description: "Generates a simplified browser support matrix for a given list of web features across major browsers.",
        inputSchema: {
            featureIds: z
                .array(z.string())
                .describe("An array of web feature IDs to include in the matrix (e.g., ['webgl', 'canvas'])."),
        },
    }, async ({ featureIds }) => {
    ...
    server.registerTool("get_min_browser_support_for_file", {
        ...
        inputSchema: {
            ...
            fileType: z
                .string()
                .optional()
                .describe("The type of the file (e.g., 'html', 'css', 'js'). If not provided, a general analysis will be performed."),
        },
        ...
        for (const featureId in detectedFeatures) {
            ...
            for (const browser of Object.keys(feature.status.support)) {
